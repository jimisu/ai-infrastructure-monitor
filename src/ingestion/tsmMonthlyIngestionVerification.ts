...
function assertMonthlyComposition(ingestedInput: MetricObservation[], ingested: MetricObservation[]) {
...
function assertCanonicalFailClosed() {
  ...
  return { missingRejected, malformedRejected }
}

function assertLatestSixMonthTrend(ingested: MetricObservation[], trend: NonNullable<ReturnType<typeof deriveTsmSignalsWithTrendConfirmation>['trend3M']>) {
...
function assertCrossConfirmation(ingestedInput: MetricObservation[], fixedTime: () => string) {
...
export function verifyTsmMonthlyIngestionParity() {
  ...
  assertMonthlyComposition(ingestedInput, ingested)

  const { missingRejected, malformedRejected } = assertCanonicalFailClosed()
  ...
  assertLatestSixMonthTrend(ingested, trend)
  assertCrossConfirmation(ingestedInput, fixedTime)
