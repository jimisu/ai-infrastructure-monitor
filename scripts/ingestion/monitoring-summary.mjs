...
export function renderMonitoringSummary(report) {
  ...
  lines.push('','| Run metric | Count |','|---|---:|',`| New facts | ${report.totals?.newFacts ?? 0} |`,`| Revisions | ${report.totals?.revisions ?? 0} |`,`| Provenance reassertions | ${report.totals?.provenanceReassertions ?? 0} |`,`| Quarantined | ${report.totals?.quarantined ?? 0} |`,`| Failures | ${report.totals?.failures ?? 0} |`,`| Warnings | ${report.totals?.warnings ?? 0} |`,'','| Coverage | Count |','|---|---:|',`| Required fact families | ${report.coverage?.counts?.requiredFactFamilies ?? 0} |`,`| INGESTED | ${report.coverage?.counts?.ingested ?? 0} |`,`| FROZEN | ${report.coverage?.counts?.frozen ?? 0} |`,`| MANUAL | ${report.coverage?.counts?.manual ?? 0} |`,`| NOT_DISCLOSED | ${report.coverage?.counts?.notDisclosed ?? 0} |`,`| MISSING | ${report.coverage?.counts?.missing ?? 0} |`,'',`**Coverage status:** ${report.coverage?.status ?? 'UNKNOWN'}`,'',`**Baseline verification:** ${report.baselineVerification?.status ?? report.verification?.status ?? 'UNKNOWN'}`,`**Proposed-state verification:** ${report.proposedStateVerification?.status ?? report.verification?.status ?? 'UNKNOWN'}`,`**Overall health:** ${report.overallHealth ?? 'UNKNOWN'}`,'','Monitoring is dry-run only; no production canonical observations are promoted by this workflow.')
  return `${lines.join('\n')}\n`
}
