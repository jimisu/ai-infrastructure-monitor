...
function verify() { return new Promise((resolve,reject) => { const child=spawn(process.execPath,['scripts/ingestion/verify-production-downstream.mjs'],{cwd:process.cwd(),stdio:'inherit'}); child.on('error',reject); child.on('exit',(code)=>code===0?resolve({productionSignals:'REGRESSION_PASSED'}):reject(Object.assign(new Error(`Downstream verification exited ${code}`),{code:'DOWNSTREAM_VERIFICATION_FAILED'}))) }) }
const result = await orchestrateIngestion({ outputRoot:path.join(process.cwd(),'data','ingestion'), dryRun, verificationRunner:verify })
