...
export const GOOG_GUIDANCE_DISCLOSURES = Object.freeze([
  { accessionNumber: '0001652044-25-000010', filingDate: '2025-02-04', reportDate: '2024-12-31', form: '8-K', primaryDocument: 'goog-20250204.htm', evidenceDocument: 'googexhibit991q42024.htm', asOf: '2024-Q4', target: '2025', shape: 'APPROXIMATE_POINT', sourceId: 'goog-2024-q4-earnings-call', publishedAt: '2025-02-04T21:30:00.000Z' },
  ...
  { accessionNumber: '0001652044-25-000056', filingDate: '2025-07-23', reportDate: '2025-06-30', form: '8-K', primaryDocument: 'goog-20250723.htm', evidenceDocument: 'googexhibit991q22025.htm', asOf: '2025-Q2', target: '2025', shape: 'APPROXIMATE_POINT', sourceId: 'goog-2025-q2-earnings-call', publishedAt: '2025-07-23T20:30:00.000Z' },
  { accessionNumber: '0001652044-25-000087', filingDate: '2025-10-29', reportDate: '2025-09-30', form: '8-K', primaryDocument: 'goog-20251029.htm', evidenceDocument: 'googexhibit991q32025.htm', asOf: '2025-Q3', target: '2025', shape: 'RANGE', sourceId: 'goog-2025-q3-earnings-call', publishedAt: '2025-10-29T20:30:00.000Z' },
  { accessionNumber: '0001652044-26-000012', filingDate: '2026-02-04', reportDate: '2025-12-31', form: '8-K', primaryDocument: 'goog-20260204.htm', evidenceDocument: 'googexhibit991q42025.htm', asOf: '2025-Q4', target: '2026', shape: 'RANGE', sourceId: 'goog-2025-q4-earnings-call', publishedAt: '2026-02-04T21:30:00.000Z' },
])
...
async function collectDisclosure({ disclosure, outputRoot, retrievedAt, fetchImpl, secUserAgent, source, acquisitionMode, fixturePath }) {
  let response, requestedUrl, finalUrl, body
  const evidence = disclosure.filingUrl ?? disclosure.officialUrl
  if (disclosure.accessionNumber) { const filing = { ...disclosure, primaryDocument: disclosure.evidenceDocument, filingUrl: disclosure.filingUrl }; const result = await collectSecFiling({ filing, cik: GOOG_SEC_ACQUISITION.cik, userAgent: secUserAgent, fetchImpl }); ({ response, requestedUrl, finalUrl, body } = result) }
  else { try { response = await fetchImpl(disclosure.officialUrl, { headers: { 'user-agent': 'AI Infrastructure Monitor data-ingestion', accept: 'text/html,application/xhtml+xml' }, redirect: 'follow' }) } catch (error) { fail('SOURCE_UNAVAILABLE', 'Alphabet IR request failed', { cause: error.message }) }; requestedUrl = disclosure.officialUrl; finalUrl = response.url || requestedUrl; if (finalUrl !== requestedUrl || new URL(finalUrl).hostname !== 'abc.xyz') fail('DOCUMENT_PROVENANCE', 'Alphabet IR document redirected unexpectedly'); body = Buffer.from(await response.arrayBuffer()) }
