...
export function assertSeparatedRoots(sourceRoot, productionRoot) {
...
export async function readJson(target, label) {
...
function expectedObservationPaths() {
  return OBSERVATION_FILES.map(observationPath)
}

function sortedLines(values) {
...
function recordsById(document, relativePath) {
  const records = new Map((document.records ?? []).map((record) => [record.recordId, record]))
  if (records.size !== (document.records ?? []).length) {
    fail('DUPLICATE_RECORD_ID', 'Canonical observations must not contain duplicate record IDs', { path: relativePath })
  }
  return records
}

function contentWithoutStatus(record) {
  return { ...record, status: undefined }
}

function allowedStatusTransition(from, to) {
  return from === to || (from === 'ACTIVE' && to === 'SUPERSEDED')
}

function assertBaselineRecordsPreserved(relativePath, baselineRecords, proposedRecords) {
  for (const recordId of baselineRecords.keys()) {
    if (!proposedRecords.has(recordId)) fail('BASELINE_RECORD_REMOVED', 'Proposed state removed a baseline record', { path: relativePath, recordId })
  }
  for (const [recordId, baselineRecord] of baselineRecords) {
    const proposedRecord = proposedRecords.get(recordId)
    if (!allowedStatusTransition(baselineRecord.status, proposedRecord.status) || stableJson(contentWithoutStatus(baselineRecord)) !== stableJson(contentWithoutStatus(proposedRecord))) {
...
function assertNewRecordsLiveBacked(relativePath, baselineIds, proposed, manifests) {
...
function assertObservationSet(fileInventory, expectedProductionHashes) {
  const expectedPaths = expectedObservationPaths()
  const actualPaths = fileInventory.filter((item) => item.path.startsWith('observations/')).map((item) => item.path)
  if (sortedLines(actualPaths) !== sortedLines(expectedPaths)) {
    fail('OBSERVATION_SET_MISMATCH', 'Reviewed state must contain exactly the five canonical observations')
  }
  if (sortedLines(Object.keys(expectedProductionHashes)) !== sortedLines(expectedPaths)) {
    fail('BASELINE_SET_MISMATCH', 'Expected production hashes must cover exactly the five canonical observations')
  }
}

async function assertObservationPromotion(relativePath, sourceRoot, productionRoot, expectedProductionHashes, manifests) {
  ...
  const baselineRecords = recordsById(baseline, relativePath)
  const proposedRecords = recordsById(proposed, relativePath)
  assertBaselineRecordsPreserved(relativePath, baselineRecords, proposedRecords)
  assertNewRecordsLiveBacked(relativePath, new Set(baselineRecords.keys()), proposed, manifests)
}

async function validateObservations(sourceRoot, productionRoot, expectedProductionHashes, manifests, fileInventory) {
  assertObservationSet(fileInventory, expectedProductionHashes)
  for (const relativePath of expectedObservationPaths()) {
    await assertObservationPromotion(relativePath, sourceRoot, productionRoot, expectedProductionHashes, manifests)
  }
}
...
function proposedInventoryFrom(productionInventory, entries) {
  ...
  return [...proposedByPath.values()].sort((a, b) => a.path.localeCompare(b.path))
}
...
export async function prepareReviewedStatePromotion({
  ...
  const bundle = {
    ...
    proposedProductionInventory: proposedInventoryFrom(productionInventory, entries),
...
function prepareArgsFromBundle(bundle) {
  return {
    sourceRoot: path.resolve(bundle.sourceRoot),
    productionRoot: path.resolve(bundle.productionRoot),
...
function assertApplyOutputPaths(source, production, rollback, deltaOutput) {
...
async function fillStaging(staging, production, source, exactDelta, proposedInventory) {
  await cp(production, staging, { recursive: true })
  await copyDelta(source, staging, exactDelta)
  if (stableJson(await inventory(staging)) !== stableJson(proposedInventory)) fail('STAGING_INVENTORY_MISMATCH', 'Staged root does not match exact proposed inventory')
}

async function exchangeStagedRoot({
  production,
  staging,
  rollback,
  displaced,
  ...
  if (stableJson(await inventory(rollback)) !== stableJson(productionInventory)) fail('ROLLBACK_SNAPSHOT_MISMATCH', 'Rollback snapshot does not match production baseline')
  ...
  try {
    ...
    if (stableJson(await inventory(production)) !== stableJson(proposedInventory)) fail('PRODUCTION_INVENTORY_MISMATCH', 'Promoted root does not match exact proposed inventory')
...
export async function applyReviewedStatePromotion({
  ...
  const fresh = await prepareReviewedStatePromotion(prepareArgsFromBundle(bundle))
  ...
  try {
    await fillStaging(staging, production, source, bundle.exactDelta, bundle.proposedProductionInventory)
    await verifyStaged(staging)
    const finalCheck = await prepareReviewedStatePromotion(prepareArgsFromBundle(bundle))
    if (finalCheck.bundleSha256 !== expectedBundleSha256) fail('PRE_APPLY_DRIFT', 'Reviewed state changed after staging verification')
    await exchangeStagedRoot({
      production,
      staging,
      rollback,
      displaced,
      productionInventory: bundle.productionInventory,
      proposedInventory: bundle.proposedProductionInventory,
      verifyProduction,
    })
