...
import { IngestionError, TSMC_MONTHLY_SOURCE, persistRawSnapshot, promoteAtomically, validateTsmcCandidates } from './tsm-monthly-lib.mjs'
...
function requireUserAgent(value) {
  if (typeof value !== 'string' || !/\S+@\S+\.\S+/.test(value)) throw new IngestionError('SEC_USER_AGENT', 'SEC_USER_AGENT must identify the application and include a monitored email address')
  return value
}
function accessionPath(value) {
  if (!/^\d{10}-\d{2}-\d{6}$/.test(value)) throw new IngestionError('ACCESSION_NUMBER', 'Invalid SEC accession ' + value)
...
function urlFor(filing) {
  ...
  return 'https://www.sec.gov/Archives/edgar/data/1046179/' + accessionPath(filing.accessionNumber) + '/' + filing.primaryDocument
}
...
export function parseSecMonthlyRevenue(snapshot,html,source=TSMC_MONTHLY_SOURCE) {
  ...
  if(new URL(snapshot.finalUrl).hostname!==TSMC_SEC_ACQUISITION.archivesHostname||snapshot.finalUrl!==p.secFilingUrl) throw new IngestionError('FILING_PROVENANCE','SEC archive URL is inconsistent')
  ...
  const common={issuer:source.issuer,companyTicker:source.ticker,period,periodType:'MONTH',sourceId:source.id,sourceUrl:snapshot.finalUrl,publishedAt:p.filingDate+'T00:00:00.000Z',retrievedAt:snapshot.retrievedAt,snapshotId:snapshot.snapshotId}
...
async function collect({filing,outputRoot,retrievedAt,fetchImpl,secUserAgent,source}) {
  ...
  return persistRawSnapshot({body:Buffer.from(await response.arrayBuffer()),source,requestedUrl:expected,finalUrl,retrievedAt,status:response.status,contentType:response.headers.get('content-type')??'',outputRoot,provenance:{cik:TSMC_SEC_ACQUISITION.cik,accessionNumber:filing.accessionNumber,formType:filing.form,filingDate:filing.filingDate,primaryDocument:filing.primaryDocument,secFilingUrl:expected,issuer:'TSMC',evidenceSourceId:source.id,evidenceDocument:TSMC_SEC_ACQUISITION.evidenceType,filedWith:TSMC_SEC_ACQUISITION.filedWith,acquisitionChannel:TSMC_SEC_ACQUISITION.acquisitionChannel}})
}
export async function ingestTsmcMonthlyFromSec({outputRoot,retrievedAt=new Date().toISOString(),fetchImpl=fetch,secUserAgent,source=TSMC_MONTHLY_SOURCE,reportingYear}) {
  ...
  const aggregate={snapshotId:'sec-accessions:'+snapshots.map((snapshot)=>snapshot.provenance.accessionNumber).join(',')}
  const promotion=await promoteAtomically({candidates,snapshot:aggregate,canonicalPath})
